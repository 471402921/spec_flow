...
- **PRD-Lite 模板**：`doc/process/prd-lite-template.md`（8 个章节的标准结构）
- **参考范例**：`doc/requirements/user-module-prd.md`（v1.3，已定稿的用户模块 PRD）
...
2. **生成初稿**：按 8 个章节逐一输出，遵循 [PRD 编写规范](references/prd-writing-guidelines.md)
   - 读取模板：`doc/process/prd-lite-template.md`
   - 参考范例：`doc/requirements/user-module-prd.md`
   - 按模板结构生成，使用范例的写作风格
