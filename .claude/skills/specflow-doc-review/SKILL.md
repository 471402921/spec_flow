...
| PRD-Lite 模板 | `prd-lite-template.md` 存在，包含 8 个章节 |
| Tech Pack 模板 | `tech-pack-template.md` 存在，包含 8 个章节 |
...
#### A4. 基础设施文档 (`specflow_plan/`)

| 检查项 | 预期 |
|--------|------|
| 项目启动记录 | 存在，阶段完成状态与 MEMORY.md 一致 |
| 部署架构设计 | 存在，内容与 `deploy/` 配置一致 |
| 技术选型 ADR | 存在，版本号与 `pom.xml` 一致 |
...
| 检查项 | 方法 |
|--------|------|
| PRD 结构 | 实际 PRD 的章节结构是否与 `prd-lite-template.md` 一致 |
| Tech Pack 结构 | 实际 Tech Pack 的章节结构是否与 `tech-pack-template.md` 一致 |
...
| 检查项 | 需要对齐的文档 |
|--------|---------------|
| 阶段完成状态 | `specflow_plan/项目启动记录.md` ↔ `MEMORY.md` 的 Progress 章节 |
