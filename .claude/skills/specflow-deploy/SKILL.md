...
# 当前部署版本
ssh specflow "cd /srv/specflow-service && git log --oneline -1"
...
# 实时跟踪 API 日志
ssh specflow "cd /srv/specflow-service/deploy && docker compose logs -f --tail=50 api"

# 查看错误日志
ssh specflow "cd /srv/specflow-service/deploy && docker compose logs api | grep -i error"

# 按 TraceId 查询
ssh specflow "cd /srv/specflow-service/deploy && docker compose logs api | grep '{traceId}'"
...
# 重建并重启 API
ssh specflow "cd /srv/specflow-service/deploy && docker compose up -d --build api"

# 重启全部服务
ssh specflow "cd /srv/specflow-service/deploy && docker compose restart"
```
...
```bash
ssh specflow "cd /srv/specflow-service && \
